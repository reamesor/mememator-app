...
import type { LaunchRecord, MemeRecord } from "@/lib/userStorage";
...
export default function CreationsModal({
  ...
  const total = launches.length + memes.length;
          ...
          {total === 0 ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-2 px-4 py-12 text-center">
              ...
              <p className="font-pixel text-sm text-zinc-500">
                No memes or lores yet. Create some in the Meme Generator or Lore pages, then send them here.
              </p>
            </div>
          ) : (
            <div className="flex flex-1 flex-col overflow-hidden sm:flex-row">
              ...
              {/* Lores (launches) section */}
              <div className="flex min-h-0 min-w-0 flex-1 flex-col">
                <div className="flex shrink-0 items-center gap-2 border-b border-zinc-700/50 px-3 py-2">
                  <FileText size={14} className="text-cyan-400" />
                  <span className="font-pixel text-[10px] font-medium text-zinc-400">Lores ({launches.length})</span>
                </div>
                <div className="flex-1 space-y-1 overflow-y-auto p-2">
                  {launches.map((l) => (
                    <button
